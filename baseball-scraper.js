...
(async function() {
  ...
  console.log(`%c[Baseball Scraper] Done! ${total} unique products, ${errors} errors`, 'color:#2196F3;font-weight:bold;font-size:14px');

  if (total === 0) {
    console.log('%c[Baseball Scraper] No products found.', 'color:#f55;font-weight:bold');
    return;
  }
  ...
  if (noImage > 0) console.log(`%c[Baseball Scraper] Warning: ${noImage} products missing image`, 'color:#fa0');

  const output = 'window.baseballProducts = ' + JSON.stringify(products, null, 2) + ';\n';
  const blob = new Blob([output], { type: 'application/javascript' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'baseball-products.js';
  document.body.appendChild(a);
  a.click();
  a.remove();

  console.log(`%c[Baseball Scraper] baseball-products.js downloaded! (${products.length} products)`, 'color:#4CAF50;font-weight:bold;font-size:14px');
})();
